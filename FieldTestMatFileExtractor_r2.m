...
for j = 1:numel(Subdir)
    ...
    for k = 1:numel(Truckfolders)
        ...
        Capability(cnt) = FieldTestMatExtractor_OldScreen_r4(DataDir,SaveDir,CleanDate1,CleanDate2,Params,tstart,tend);
        cnt =cnt+1;
...
%         set(gca,'XTickLabel',datestr(iupr_startTime))
        ...
        set(gca,'XTickLabel',Capability(i).KeyCycles)
        ...
        n = length(Capability(i).Old_Mot);
        g = cat(1,repmat({'old cal'},n,1),repmat({'FED decision'},n,1),repmat({'new cal'},n,1));
        boxplot([Capability(i).Old_Mot;Capability(i).TimetoDec;Capability(i).New_Mot],g,...
